...
local overlay = display.newImage("images/overlay.png")
overlay:toBack()
...
local function movementListener(event)
                ...
                --todo: check to make sure that this isn't going to cause problems down the line
...
function tapListener( event )
    ...
    if (event.numTaps >= 2 ) then
        ...
        if tapCounter == 0 then
            ...
            self.yScale = 4
            self:removeEventListener("touch", movementListener)
            mainGroup:insert(self)  
            ...
            self.yScale = 1
            scrollView:insert(self)
            self:addEventListener("touch", movementListener)
            scene:AdjustScroller()
            scrollView.isVisible = true
            overlay:toBack()
